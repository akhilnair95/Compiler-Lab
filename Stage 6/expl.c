...
void typeCheck(struct Tnode* c1,struct Tnode* c2,struct Tnode* c3,int type){
	
	// Check if the non NULL Arguments specified are of type 'type' else flag error 
	
	if(type == TYPE_BOOL){
					// for IF and WHILE
			
					if(c1 != NULL){
						if(getType(c1) != TYPE_BOOL)
							error("Type mismatch error");
						}
						
					
					if(c2 != NULL){
						if(getType(c2) != TYPE_BOOL)
							error("Type mismatch error");
						}
					
					
					if(c3 != NULL){
						if(getType(c3) != TYPE_BOOL)
							error("Type mismatch error");
						}				
		}
		
	else{	
			// for ASG , READ (for arrays)
			
					if(c1 != NULL){
						if(getType(c1) != TYPE_INT)
							error("Type mismatch error");
						}
						
					
					if(c2 != NULL){
						if(getType(c2) != TYPE_INT)
							error("Type mismatch error");
						}
					
					
					if(c3 != NULL){
						if(getType(c3) != TYPE_INT)
							error("Type mismatch error");
						}
	}				
}
